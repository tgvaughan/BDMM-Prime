...
- Parallelization Factor :: This determines the fraction of
  all of the tree nodes a subtree must contain for its tree prior
  calculation to be assigned to a different thread. (Does nothing if
  the Parallelize option is unchecked.)
