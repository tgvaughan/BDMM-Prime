...
filtering algorithm (PREPRINT).  For a given sampled tree and set of
model parameters, this algorithm simulates an ensemble of possible
...
weighted distribution.  By applying this process to a subsample of
the states visited by the MCMC chain, we are left with a set of
...
using the R packages dplyr and ggplot2.
...
simulation algorithm allows more than one event to occur at a given
time.)
