...
#+begin_quote
Click the arrow to the left of ~Tree.t:h3n2_2deme~ to make the tree prior options visible.
Then, Scroll down to end of the tree prior section and find the table associating tree leaves
...
"Enable logger" box.  (NB: in recent versions of BDMM-Prime, typed
tree loggers are only displayed when two or more types are present in
the model.)  By default edge-typed tree logging is enabled, but
