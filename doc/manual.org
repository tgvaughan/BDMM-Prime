...
- use of stochastic mapping for efficient sampling of ancestral types,
...
Because of the many changes that were required in making this transition,
BDMM-Prime is completely incompatible with BDMM itself.  (The original
package will be maintained separately to ensure that BEAST 2 XMLs and packages
that depend on it remain usable.)

** Acknowledgements and Citations
:PROPERTIES:
:CUSTOM_ID: id-2-Introduction-Acknowledgements-and-Citations
:END:

As this is a fork of [[https://github.com/denisekuehnert/bdmm][BDMM]], BDMM-Prime owes its existence to the
...
- Vaughan and Stadler, /Bayesian phylodynamic inference of multi-type population trajectories using genomic data/, Molecular Biology and Evolution 42:msaf130 (2025), [[https://doi.org/10.1093/molbev/msaf130][doi:10.1093/molbev/msaf130]]. (BDMM-Prime, trajectory inference)
- Scire et al., /Robust Phylodynamic Analysis of Genetic Sequencing Data from Structured Populations/, Viruses 14:1648 (2022), doi:10.3390/v14081648. (BDMM)
- Kühnert et al., /Phylodynamics with Migration: A Computational Framework to Quantify Population Structure from Genomic Data/, Molecular Biology and Evolution 33:2102-2116 (2016), doi:10.1093/molbev/msw064. (BDMM)
...
3. The latest version of BDMM-Prime should appear in the list of packages:
...
#+begin_quote
Click the arrow to the left of ~Tree.t:h3n2_2deme~ to expand the tree prior section.
...
We now need to define how lineages change type in our multi-type
...
There are a few other options which can be set here, in particular the
process conditioning and the prior probabilities for the starting
types, but we will leave these set to the default values for this
tutorial.  (These options are discussed in detail in the [[Model
specification using BEAUti]] chapter.)

# Finally, we need to decide how we want to condition our birth-death
# process. One aspect of this is decide whether we want to model the
# process as starting at some defined point in the past which is older
# than the tree (often called the "origin" of the process), or whether
# we want the start of the process to conincide with the root of our
# tree. Further, in the instance that we choose to condition on the
# process origin, we can also decide to condition our birth-death tree
# prior on having obtained at least one sample (confusingly called
# "conditioning on survival").

# The meanings, pros and cons for these options are discussed in more detail in the [[Process Conditioning]] section.  For now, 
# we will assume the process starts at the origin, the time of which we will estimate,
# and we will condition on "survival" (i.e. sampling) of at least one individual.

# #+begin_quote
# Observe and do not modify the default process conditioning options displayed
# at the end of the parameterization section of the tree prior.
# #+end_quote

# file:./figures/tutorial_conditioning.png
...
With the multi-type model itself configured, what remains is to define priors for
...
| =becomeUninfectiousRateEpi.t:h3n2_2deme= | LogNormal(4,0.5) |
| =migrationRateEpi.t:h3n2_2deme=          | Exp(0.1)         |
| =samplingProportionEpi.t:h3n2_2deme=     | Unif(0,1)        |
| =Process Length=                         | Gamma(5,2)       |
|------------------------------------------+------------------|
#+end_quote

The Process Length parameter here describes the duration of the birth-death process which,
due to our choices above, corresponds to the time between the "origin" and the time of the most recent sample in our alignment.
...
- typedTreeLogger :: A logger which uses stochastic mapping to
  ...
  (/Only displayed for models containing more than 1 type./)
...
"Enable logger" box.  By default edge-typed tree logging is enabled, but
...
*Important:* Be sure to define all model linkages between partitions /before/
configuring the BDMM-Prime tree prior.  Modifying linkages later on can result
in broken BEAST 2 models.
...
are initially given the same type, =NOT_SET=. (There is nothing
...
problems may occur if strings contain spaces.
...
*** Leaves with unknown type
:PROPERTIES:
:CUSTOM_ID: id-3-Defining-tip-types-Model-specification-using-BEAUti-Leaves-with-unknown-type
:END:

Sometimes you may not know to which type a tip should be assigned. You
can ask BDMM-Prime to infer the type of these tips.

To do this, ensure the "Allow tip type ambiguities" option above the tip type table
is selected, as indicated by the red arrow in the following image:

[[./figures/unknown_types.png]]

If the tip type is completely unknown, you can set the type to a "?"
which indicates that the type is unknown. On the other hand, if you only wish to
allow the tip to take a on values from a subset of the possible types, enter the
names of the possible types separated with a '|' character.  Example use of this
notation is shown in the cells indicated by the blue arrows in the image.

Beware that designating all of your tip types as unknown will
result in unidentifiability with respect to permutation of the mapping
between types and rates. To avoid this you'll need to impose some
additional symmetry-breaking constraints, e.g. =birthRate[0]>birthRate[1]=.
(Imposing these is currently outside the scope of this manual. If
you'd like to hear more about this please get in touch with me.)

*** Defining additional types
:PROPERTIES:
:CUSTOM_ID: id-3-Defining-tip-types-Model-specification-using-BEAUti-Defining-additional-types
:END:

It is occasionally the case that there are additional types which are
absent from your sampled data which you nonetheless wish to include in
your model.  For instance, in an (E)xposed-(I)nfectious
epidemiological model your samples likely correspond only to
infectious individuals.  To accommodate this, you can add a
comma-delimited list of type names to the "Additional types" text field below the
tip type assignment table:
[[./figures/additional_types.png]]
(You can also include the names of sampled types in this list: duplication has no effect.)

*** Type indices
:PROPERTIES:
:CUSTOM_ID: id-3-Defining-tip-types-Model-specification-using-BEAUti-Type-indices
:END:

Internally, BDMM-Prime uses numerical indices to refer to individual types.
While you are generally shielded from this detail - most type-dependent
input and output is labeled exlicitly using the type names provided by the
type trait set, it is occasionally necessary to know these indices.

For $M$ types, the type indices range from 0 through to $M-1$. They
correspond to the type names sorted in (generalized) alphabetical
order.  Thus in the tutorial analysis, the type indices are 0 and 1 for
the types with labels HongKong and USA, respecticely.

If you're ever confused about the type/index mapping, it is given explicitly
in BEAUti directly underneath the Additional Types input at the base of the
the type trait set input editor:

[[./figures/typeIndices.png]]
...
(generalized alphabetical) order. (See the previous section on type indices.)
...
to assume - that an outbreak was seeded by travel cases from other
locations.  In such cases you can modify the probability values to reflect
these prior assumptions, although take care that the values still sum to 1.
...
(The rho sampling, migration and process length parameters are
identical to those described for the canonical parameterization.  The
removalProbability is fixed to zero in the FBD parameterization.)
...
By default, the change times/ages are not estimated: they retain their chosen
...
times/ages during the MCMC.  If you choose this, be sure to place
appropriate upper and lower bounds using the corresponding prior.  The default
distribution is uniform over possible change times/ages, equivalent to a
flat Dirichlet prior on the intervals between the times/ages.
...
allowing you to determine at a glance which epoch a given sample is
...
:PROPERTIES:
:CUSTOM_ID: id-2-Model-specification-using-BEAUti-Process-Conditioning
:END:

The process conditioning block allow you to adjust how the birth-death
tree generation process is conditioned. By default it appears as follows:

[[./figures/conditioning.png]]

The following options can be set:

- Condition on Root ::  Enabling this option causes the process length
  to be fixed to the age of the root of the tree relative to the end
  of the process, and to appropriately condition the tree prior on this root.
  In the case of models which allow for sampled ancestors, this option
  additionally imposes a condition that no sampled ancestors appear on
  the "stem edge" of the tree. (I.e. the root node must be a divergence.)

- Condition on Origin ::  Enabling this option causes the process length
  to be fixed to the age of the "origin" of the birth-death process,
  which is in general older than the age of the root. The value of the
  adjacent checkbox determines whether this parameter should be itself
  estimated.  This option is enabled by default, as it is the most common
  choice.

- Initial age of Origin :: When "Condition on Origin" is selected, this
   can be used to set the (initial) value of the origin age parameter.

- Condition on Survival :: When "Condition on Origin" is selected,
  enabling this option causes the tree prior to include a term which
  conditions the tree probability on having at least one sampled
  individual in the dataset.

To place a parametric prior on the process length---regardless of
whether it corresponds to the tree root age or the origin age---use
the parametric prior selection option labeled
"processLengthBDMMPrime" toward the bottom of the page.

*Warning:* If you select "Condition on Root", take care to avoid
defining a separate MRCA prior for the root since one is already
defined for "processLengthBDMMPrime".
...
- Method :: This determines which of BDMM-Prime's two calculation engines is
  used to compute the tree prior.  Both compute the same quantity, but the
  flow engine is substantially faster on large trees with several types.
  Under the default "auto" setting, BDMM-Prime chooses between them for you,
  and you should usually leave this as-is.
...
- Parallelization Factor :: When parallelization is enabled, this
  ...
  (This applies to the classic engine only. The flow engine uses its own
  threshold, which can be customized in the XML as described in the reference
  section below.)
...
check/uncheck the "Enable Logger" checkbox. (**Note: these input editors
are only displayed when your model specifies more than one type!**)
...
:PROPERTIES:
:CUSTOM_ID: id-2-Sampling-Latent-Variables-Population-trajectories
:END:

Multi-type population trajectories are sampled using a phylogenetic
particle filtering algorithm based on the method described by
...
:PROPERTIES:
:CUSTOM_ID: id-3-Population-trajectories-Sampling-Latent-Variables-Plotting-sampled-trajectories
:END:
...
#+begin_src R
  ggplot(gridded_traj,
         aes(x=age, y=med, ymin=low, ymax=high, col=factor(type), fill=factor(type))) +
...
#+begin_src R
  ggplot(gridded_traj %>%
         ...
         mutate(Location=recode(factor(type),
                           "0"="Hong Kong",
                           "1"="New Zealand")),
         aes(x=Date, y=med, ymin=low, ymax=high, col=Location, fill=Location)) +
      geom_ribbon(alpha=0.5) +
...
=bdmmprime.facade.BirthDeathMigrationDistribution= object representing
...
#+begin_src xml
  <distribution spec="bdmmprime.facade.BirthDeathMigrationDistribution">
    <!-- Inputs -->
  </distribution>
#+end_src
...
- =method= (=String=, default auto) :: This selects the engine used to
  compute the tree probability, and is one of =auto=, =classic= or =flow=.
  Under =auto=, the classic engine is used for single-type analyses and for
  trees with fewer than 100 leaves, and the flow engine is used for
  everything else.
...
- =startTypePriorProbs= (=Simplex=, required for multiple types) :: This is a object
  of type =Simplex=/=SimplexParam= whose dimension is the number of types and
...
- =finalSampleOffset= (=RealScalar=, default 0) :: If provided, this determines
...
for debugging and/or tuning the calculation performance, and are understood by
both engines:
...
The following inputs have an effect only when the classic engine is in use.
They are ignored by the flow engine:
...
- =storeIntegrationResults= (=Boolean=, default false) :: Indicates whether
  to retain the results of the ge(t) integration so that they can be used by
  other models.

Likewise, the following inputs have an effect only when the flow engine is in
use, and are ignored by the classic engine:

- =initialMatrixStrategy= (=String=, default average_inverse) :: The strategy
  used to choose the initial state of the flow integration on each interval,
  and is one of =identity=, =random=, =average_inverse=.
- =useInverseFlow= (=Boolean=, default false) :: Indicates whether to use the
  inverse flow algorithm.  This is faster, but can lead to greater numerical
  instability.
- =seed= (=Integer=, default 3215) :: The random seed used to draw the initial
  state under the =random= initial matrix strategy.
- =minimalSubtreeSizeForParallelization= (=Integer=, default 64) :: The
  minimum size (number of nodes) the smaller of a pair of subtrees must have
  in order for their calculations to be carried out in separate threads.
  This is the flow engine's counterpart to =parallelizationFactor=.
- =maxConditioningNumber= (=Double=, default 1e8) :: The largest condition
  number tolerated before an interval is split into smaller ones.
- =useLoucaPennellIntervals= (=Boolean=, default false) :: Indicates whether
  to use the interval heuristic introduced by Louca and Pennell.
...
values to the internal representation of birth-death models used by =BirthDeathMigrationDistribution=.
...
- =processLength= (=RealScalar=, required) ::
  ...
  to true.  If the former, supplying an "origin" parameter is usual.
...
- =skylineValues= (=RealVector=, required) ::
- =changeTimes= (=RealVector=) ::
  Each of this pair of inputs takes a =RealVector= (such as a =RealVectorParam=).
  ...
  The dimension $N$ of the skylineValues =RealVector= or =RealVectorParam=, in combination with the
  dimension $K$ of the changeTimes =RealVector= or =RealVectorParam= (0 if this input is not set),
...
- =times= (=RealVector=) ::
  ...
  The dimension $N$ of the values =RealVector= or =RealVectorParam=, in combination with the
  dimension $K$ of the times  =RealVector= or =RealVectorParam=,
...
- =allowAmbiguousTypes= (=Boolean=) :: If enabled, the types assigned
  to certain taxa may be treated as "ambiguous", i.e. incompletely
  known.  How such ambiguities are specified is determined by the
  following pair of inputs.
  
- =unknownTypeIdentifier (=String=, default "?") :: When
  =allowAmbiguousTypes= is true, taxa whose type trait value matches
  this string will be regarded as having a completely "unknown" type. That is,
  the true type of the taxon may be any one of those available in the model, with
  prior probabilities determined by the multi-type birth-death process.
  
- =ambiguousTypeDelimiter= (=String=, default "\|") :: This string represents a
  regular expression. When =allowAmbiguousTypes= is true, this regular expression
  will be used to split taxon type trait values into substrings, which are then
  interpreted as the possible types that taxon can take.  For example, the default
  regexp would cause the type trait value "USA|NewZealand" to indicate that the
  corresponding taxon could belong to either of these two demes.
...
- =bdmmTreePrior= (=BirthDeathMigrationDistribution=) ::
  Reference to the =bdmmprime.facade.BirthDeathMigrationDistribution= used in
  the analysis.
...
- =bdmmDistrib= (=BirthDeathMigrationDistribution=) :: 
  ...
  needed to sample the ancestral type distribution.  If used, this must be a
  reference to the =bdmmprime.facade.BirthDeathMigrationDistribution=.
...
- =startTypePriorProbs= (=Simplex=, required) ::
...
- =finalSampleOffset= (=RealScalar=, default 0) :: 
...
- =finalSampleOffset= (=RealScalar=, default 0.0) ::
...
- =bdmmDistrib= (=BirthDeathMigrationDistribution=) :: 
  One of these two inputs must be provided, as these provide the parameters
  needed to sample the population dynamics.  If used, this must be a
  reference to the =bdmmprime.facade.BirthDeathMigrationDistribution=.
...
#+begin_src xml
  <distribution spec="bdmmprime.util.priors.SmartZeroExcludingRealIID">
    <!-- Inputs -->
  </distribution>
#+end_src

This prior is intended as a drop-in replacement for the standard
"beast.base.inference.distribution.IID" used to define parametric priors
on ~RealVectorParam~s. Indeed, it takes almost the same inputs:
- =param= (=RealVector=, required) ::
  Parameter to which prior is applied.
- =distr= (=ScalarDistribution=, required) ::
  Specifies which parametric distribution to apply, e.g. ~LogNormal~, ~Exponential~, etc.
- =classesToExclude= (=RealVector=) ::
  Elements possessing any of these values will be excluded from the prior calculation.

Unlike the standard ~IID~ distribution though, ~SmartZeroExcludingRealIID~ does the following:
...
at least as it applies to a ~RealVectorParam~.  It takes the following inputs:
- =parameter= (=[RealVectorParam]=, at least one required) ::
...
that like the ~SmartZeroExcludinRealIID~, the ~SmartScaleOperator~ groups identical
...
- =finalSampleOffset= (=RealScalarParam=, required) ::
...
- =finalSampleOffset= (=RealScalar=, required) :: 
...
- =internalNodeSpacing= (=RealScalar=, default 1.0) :: 
...
Population sizes are reported by rows with the "variable" column set
to "N".  Each such row reports the population size associated with the
type index given in the "type" column and the size in the "value"
column. The size corresponds to the time immediately at and after the
time given in the "t" column. (Respectively, the age immedately before
the age given in the "age" column.)
...
BDMM-Prime is free software.  It is made available to you under the
terms of the [[https://www.gnu.org/licenses/gpl-3.0.html][GNU General Public Licence version 3]] or, at your option,
any later version. Unlike many software licenses, this is designed to
give you, the user, the freedom to use the software as you wish -
including making and publishing modifications - provided you also
extend the same courtesy to other users.
