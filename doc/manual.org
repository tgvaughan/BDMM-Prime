...
flexibility.  At the top level, the BDMM-Prime tree prior takes only
the following four main elements:
1. The *tree*,
2. *Tip type information* (if available) and *starting type probabilities*,
3. A "*parameterization*" object defining how the model is to be parameterized, and
4. Miscellaneous *tuning parameters*.
...
parameterizations.  With only one minor exception for the FBD parameterisation
