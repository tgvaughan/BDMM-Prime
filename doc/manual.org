...
and sum to 1). We advise against doing this, as doing so is completely
equivalent to simply setting a uniform prior in the first place.  This
option is provided to allow for compatibility with previous
implementations of this model, and beware that the values so estimated
do not represent the posterior probabilities of the starting state.
