...
By default the prior probabilities are given a uniform prior.  In many
...
- Birth Rate ($\lambda_{ii}$) :: This is the (per individual per unit time) rate at which
  each individual in the population produce new individuals
  of their own type.
- Death Rate ($\mu_i$) :: Similarly, this is the rate at which individuals in the population
  are removed from the population. 
- Sampling Rate ($\psi_i$) :: This is the rate at which individuals are "sampled". A non-zero
...
- Removal probability ($r_i$) :: This is the probability with which sampled individuals
  are also removed from the population.
...
- Birth Among Demes ($\lambda_{ij}$) :: $\lambda_{ij}$
...
The assumption behind the "epidemiological" parameterization is that individuals
in the model represent infected individuals in a population. The basic birth, death and
death and sampling parameters are
...
- Removal probability ($r_i$) :: This has the same definition as in the canonical parameterization.

The type-change reactions are parameterized by
- Migration rate ($m_{ij}$) :: This has the same definition as in the canonical parameterization.
