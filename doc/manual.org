...
#+begin_quote
1. Open the "File" menu and select "Save". Choose a sensible location in
your filesystem in which to save the analysis file, which should be
named ~h3n2.xml~.  (By default the location will be the examples/
folder from which you loaded the sequences: you'll want to choose
somewhere else, perhaps a folder on your desktop.)
...
In the following section we'll delve into the BDMM-Prime interface
in more detail.  Unlike the tutorial, this section is written in a
more general way rather than considering a specific data set.

** The BDMM-Prime Tree Prior

Using BDMM-Prime amounts to selecting the BDMM-Prime tree prior when
choosing priors for a particular analysis.
