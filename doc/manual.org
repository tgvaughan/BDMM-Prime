...
- Engine :: This determines which of BDMM-Prime's two calculation engines is
...
Finally, the choice between the two engines is itself made by a single input:

- =engine= (=String=, default auto) :: This selects the engine used to
...
- =bdmmTreePrior= (=BirthDeathMigrationDistribution=) ::
  Reference to the =bdmmprime.distribution.BirthDeathMigrationDistribution= used in
  the analysis.
...
- =bdmmDistrib= (=BirthDeathMigrationDistribution=) :: 
  ...
  reference to the =bdmmprime.distribution.BirthDeathMigrationDistribution=.
