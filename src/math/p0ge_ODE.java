...
public class p0ge_ODE implements FirstOrderDifferentialEquations {
	...
	public static void testCorrelations(){
		...
		for (double i = 1.5; i<5; i+=0.25){
			...
			FirstOrderIntegrator integrator = new ClassicalRungeKuttaIntegrator(.00001);
			FirstOrderIntegrator integrator2 = new DormandPrince54Integrator(1.0e-6, 1., 1.0e-320, 1.0e-20);//new ClassicalRungeKuttaIntegrator(.01); //
			FirstOrderIntegrator integrator3 = new ClassicalRungeKuttaIntegrator(.000001);
			FirstOrderIntegrator integrator4 = new ClassicalRungeKuttaIntegrator(.01);
	...
	public static void main(String args[]){
		...
		SmallNumber[] p0sn = SmallNumber.getSmallNumbers(p0);
		ScaledNumbers p0scaled = SmallNumberScaler.scale(p0sn, EquationType.EquationOnP);

		p0_ODE p_ode = new p0_ODE(b,new Double[]{1.,1.}, d,s,M, 2, 1, new Double[]{0.}, p0scaled.getScalingFactor()[0], true);
		p0ge_ODE pg_ode2 = new p0ge_ODE(b,new Double[]{1.,1.}, d,s,M, 2, 1, T,
				new Double[]{0.}, p_ode, Integer.MAX_VALUE,augmented, true);
		pg_ode2.p_integrator =  new ClassicalRungeKuttaIntegrator(.01);
		SmallNumber[] res = pg_ode2.getPSmallNumber(0, false, null);
		System.out.println(SmallNumber.toString(res));
		...
		p0ge_ODE pg_ode = new p0ge_ODE(b,new Double[]{1.,1.}, d,s,M, 2, 1, T, new Double[]{0.}, p_ode, Integer.MAX_VALUE,augmented, 0, false);
